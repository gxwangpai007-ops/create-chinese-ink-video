...
当同一画面包含多个 Logo、人物、产品或叙事区域时，Skill 不再为每个区域手写独立动画：
...
`render_multi_region_ink.py` 读取母版、无主体环境底板、宣纸底图和 JSON 配置，统一调度环境路径、区域墨团、形成顺序和锁形时间。仓库提供 `multi-region-ink-template.json` 作为起点。
