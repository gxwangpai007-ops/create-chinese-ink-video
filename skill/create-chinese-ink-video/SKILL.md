...
description: 将主题、文稿、参考图片或参考视频转化为中国水墨风格的关键帧、分层 Layer Pack、动态样片和完整视频，并通过方向审批、双参考职责分离、强水墨静态压力测试、晕染形成语法、连续流体墨场显影、纤维状缺口墨团生长、多区域联合显影、瓶底浅水墨场与倒影折射、主体出场审计、平滑低清预览、局部重渲染和最终 QA 保持跨镜头一致性。用户要求“中国水墨视频”“国风水墨动画”“水墨晕染视频”“墨在水里散开”“喷气或墨流经过后画面显现”“瓶底水墨波纹”“水中倒影随波纹晃动”“C 形烟雾或随机墨团逐渐显影”“纤维状墨迹生长”“多个 Logo 或主体沿同一墨脉依次显现”“现代城市水墨”“参考这张图的质感重绘另一个场景”“把画中元素拆层形成”“让人物、汽车、建筑通过墨色渗化和笔触形成”，或需要修改、剪辑、排查卡顿、白洞及重复镜头时使用。
---
...
- 产品接触面上的水墨波纹与水中倒影使用同一浅水高度场；产品实体最后锁定，不得随波场扭曲。
- 多区域画面使用一套共享环境到达场连接各主体；每个区域只负责局部墨团和后段精确锁形，不得各自独立淡入。
...
用户要求产品瓶底或物体接触面产生扁平水墨波纹，并让水中倒影随波纹自然晃动时，完整读取 [shallow-water-ink-reflection.md](references/shallow-water-ink-reflection.md)。使用 `render_shallow_water_ink_reflection.py` 让同一椭圆浅水高度场驱动可见墨峰、湿边、波面亮暗和倒影折射；产品实体必须最后锁定合成。
...
同一画面包含两个以上 Logo、人物、产品或叙事区域，并要求它们沿同一墨脉依次形成时，完整读取 [multi-region-ink-orchestration.md](references/multi-region-ink-orchestration.md)。从 `assets/multi-region-ink-template.json` 建立项目配置，使用 `render_multi_region_ink.py` 让共享环境到达场、局部纤维墨团、留白保护和精确主体锁定按同一时间轴执行；不要为每个区域手写独立淡入。
